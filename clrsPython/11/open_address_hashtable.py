...
def double_hashing_hash_function(h1, h2):
	"""Return a function that probes slot positions 
  offset by a second hash function.
	Parameters:
...
class OpenAddressHashTable:
	...
	def __init__(self, m, h1, h2=None, get_key_func=None):
		...
		linear-probing -- True if the hash function uses linear probing, 
	  False otherwise.
    ...
    # or have searched every element.
	...
	def search(self, k):
		...
		if self.m == 0:
			return None
		i = 0 
		# Continue probing until finding an element with key k, 
	...
	def double_hashing_hash_delete(self, k):
		"""Delete an object from the hash table by 
	  replacing it with self.deleted."""
	...
	def linear_probing_hash_delete(self, k):
	  ...
	  knowing that linear probing is used."""
		q = self.search(k)  # index of the slot where the object is located
		if q is None:  # if not found
			raise RuntimeError("Cannot delete: " + str(k) + " is not in hash table")
		...
		while True:
			...
			while True:
				...
				if self.g(k_prime_key, q, self.m) < self.g(k_prime_key, 
											   q_prime, self.m):  
					break
...
if __name__ == "__main__":
	...
	objs = [KeyObject("Asparagus", 2), KeyObject("Banana", 3), 
		  KeyObject("Carrot", 5),
		  KeyObject("Dill", 7), KeyObject("Escarole", 11), 
	    KeyObject("Fava bean", 13),
			KeyObject("Garlic", 19), KeyObject("Haricot", 23), 
	    KeyObject("Iceberg lettuce", 25),
			KeyObject("Jicama", 29)]
	for i in objs:
		print("h(" + str(i) + ") = " + \
	...
	def aux_hash_func(k):
		return k % 701
	hashtable4 = OpenAddressHashTable(15, hashpjw, 
								   aux_hash_func, KeyObject.get_key)
	for i in range(14):
